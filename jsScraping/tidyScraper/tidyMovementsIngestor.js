...
async function processCSVAndInsertIntoDatabase(filePath) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    console.log({
      ...
      sta: parseTime(row[7]),
      ata: parseTime(row[8]),
      toff: parseTime(row[9]),
      tdwn: parseTime(row[10]),
      atd_offset: parseInt(row[11]) || null,
      ata_offset: parseInt(row[12]) || null,
      blk_offset: parseInt(row[13]) || null,
      taxi_toff: parseInt(row[14]) || null,
      taxi_ata: parseInt(row[15]) || null,
      delay_codes: row[16] || "",
      cnl: row[17] || "",
    });
    console.log("-----------------------------------");
